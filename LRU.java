...
public class LRU extends SubstitutionAlgorithm {
    
    LRU(int numOfFrames, List<Integer> readSequence, String prefix){
        ...
        for (int i = 0; i < readSequence.size(); i++){
            if (!memoryContains(readSequence.get(i))){
                ...
                if (memory.size() == frameCount) {
                    ...
                    if (DEBUG)
                        System.out.println("REMOVING " + farthestPage.pageNumber);
